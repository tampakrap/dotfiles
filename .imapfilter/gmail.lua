...
gmail_unseen:match_from('(accounts@oreilly|(invitation-)?do-not-reply@(trello|stack(exchange|overflow))|admin@transifex|reddit@reddit|no-reply@(dropbox|socghop\\.appspotmail|lanyrd|susestudio|gitlab\\.uservoice)|((d\\+MTA4MDQ2NTk2NjI3OTg3NjgyMjIw-.*@docs|no(-)?reply(@(accounts|checkout|cbg\\.bounces|wallet)|-.*@plus))\\.|(mail|drive)-noreply@)google|account-security-noreply@account\\.microsoft|notice-5622092@no-ip|gitlab@gitlab|ebay@ebay|(issue-updates|tickets|noreply|forge)@puppetlabs|notification\\+ka4k2g4a@facebookmail|(miles\\.bonus|echeckin|noreply|boardingpass)@aegeanair|service@(intl\\.paypal|payu)|webmaster@novell|mantis@public\\.kitware|support@olx|donotreply@gravatar|(passwordhelp|donotreply)@wordpress|alerts@citibank|notify@twitter|noreply(-www@services\\.|@(notifications\\.)?)swiss|youtube|(info@advisory\\.|itinerary@)ryanair|(bme@mailer\\.|customer\\.service@)booking|prague_harfa_(admin|msm)@jatomi)\\.com|(notify@keybase|support@drone)\\.io|(support@cacert|accounts@fedoraproject|donotreply@rubygems|noreply@(redmine|wordpress)|no-reply@(persona|mozillians)|.*@bitbucket|bugzilla_noreply@kde)\\.org|(no_reply@gopay|(info|robot)@jidloted|zakaznici@damejidlo|noreply@c(sas|inemacity)|eshop_noreply@dpp|obchod@ticketportal|ticket@ticketpro|letenky@pelikan|reservation@previo|express@studentagency|info@goout)\\.cz|no-reply@jetpack\\.me|(no-reply@vodafonecu|info@flowernet|billing@ziz|(billing|info|support)@papaki|hmaster-info@ics\\.forth)\\.gr|(fahrkartenservice@bahn|(info@|puppetcamp\\.teilnehmer@rt\\.)netways)\\.de'):move_messages(gmail['logs'])
