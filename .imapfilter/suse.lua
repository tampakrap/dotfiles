...
logs = suse_unseen:match_from('GWAVA@novell\\.com') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}') + suse_unseen:match_field('RT-Ticket', 'SUSE Ticket #[0-9]{5,6}') + suse_unseen:contain_from('swamp_noreply@suse.de'):contain_to('tchatzimichos@suse.cz')
logs:move_messages(suse['logs'])
bugzilla = suse_unseen:contain_from('bugzilla_noreply@novell.com')
bugzilla:match_to('tampakrap@opensuse\\.org|tchatzimichos@suse\\.com'):move_messages(suse['logs/bugzilla'])
...
suse_unseen:match_to('DL-(TAG|SU)-(ALL|All|EMPLOYEES|Employees)(\\.iList\\.INTERNET)?@ilist\\.attachmategroup\\.com'):move_messages(suse['newsletters'])
