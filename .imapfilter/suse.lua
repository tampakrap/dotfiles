...
infra_logs = infra:match_from('((netapp0[1-3]|rt-count)@suse|root@crick.suse)\\.de|(acmon1\\.no-reply|netapp0[5-6])@suse\\.cz|(asupprod@netapp|ShipmentNotificationDVOnline@intel)\\.com') + infra:match_to('rd-adm-svn@suse\\.de') + suse_unseen:match_from('scz-probe[1-3]@(suse|novell)\\.com|keepalived@(sparta|slavia)\\.suse\\.cz|netbolz-srv1-c7@suse\\.cz')
...
logs = suse_unseen:match_from('(HelpDesk@attachmategroup|((istdirteam|GWAVA|gwava|webmaster|pwnotify|SSC-EMEA)@|(psoft@(sybok|sarek)|WF-BATCH@eccprd1)\\.provo\\.)novell|helpdesk@netiq|support@ovationincentives|mailer@doodle|no-reply@accounts\\.google|(eServiceDesk|ssc-emea|guest|AssetManagement)@microfocus|(orders|noreply)@eventbrite)\\.com|(swamp|orthos)_noreply@suse\\.de') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}')
...
newsletters = suse_unseen:match_to('DL-(Prague|PRAGUE|TAG|SU(-(ALLFY16|ENG))?|MicroFocusInternational|MICROFOCUSINTERNATIONAL)(-(A[lL]{2}|EMP(LOYEES)?|Employees|Office|OFFICE))?(\\.iList\\.INTERNET)?@(ilist\\.attachmategroup\\.com|ILIST\\.ATTACHMATEGROUP\\.COM)') + suse_unseen:contain_from('einfo@suse.com')
