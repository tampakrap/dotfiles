...
logs = suse_unseen:match_from('(HelpDesk@attachmategroup|((istdirteam|GWAVA|gwava|webmaster|pwnotify|SSC-EMEA)@|(psoft@(sybok|sarek)|WF-BATCH@eccprd1)\\.provo\\.)novell|helpdesk@netiq|support@ovationincentives|mailer@doodle|no-reply@accounts\\.google|(eServiceDesk|ssc-emea)@microfocus|(orders|noreply)@eventbrite)\\.com|(swamp|orthos)_noreply@suse\\.de') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}')
