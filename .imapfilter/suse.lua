...
infra_logs = opensuse_list('admin-auto') + infra:match_from('((netapp0[1-2]|rt-count)@suse|root@crick.suse)\\.de|netapp0[5-6]@suse\\.cz|(asupprod@netapp|ShipmentNotificationDVOnline@intel)\\.com') + infra:match_to('rd-adm-svn@suse\\.de') + suse_unseen:match_from('scz-probe[1-3]@(suse|novell)\\.com|router[ab]@suse\\.cz')
...
logs = suse_unseen:match_from('(HelpDesk@attachmategroup|((istdirteam|GWAVA|gwava|webmaster|pwnotify|SSC-EMEA)@|(psoft@(sybok|sarek)|WF-BATCH@eccprd1)\\.provo\\.)novell|helpdesk@netiq|support@ovationincentives|mailer@doodle|no-reply@accounts\\.google|eServiceDesk@microfocus)\\.com|(swamp|orthos)_noreply@suse\\.de') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}')
