...
infra_logs = opensuse_list('admin-auto') + infra:match_from('((netapp0[1-2]|rt-count)@suse|root@crick.suse)\\.de|netapp0[5-6]@suse\\.cz|(asupprod@netapp|ShipmentNotificationDVOnline@intel|scz-probe3\\.scz\\.novell\\.com@novell)\\.com') + infra:match_to('rd-adm-svn@suse\\.de')
...
logs = suse_unseen:match_from('(HelpDesk@attachmategroup|((istdirteam|GWAVA|gwava|webmaster|pwnotify)@|(psoft@(sybok|sarek)|WF-BATCH@eccprd1)\\.provo\\.)novell|helpdesk@netiq|support@ovationincentives|mailer@doodle|no-reply@accounts\\.google)\\.com|(swamp|orthos)_noreply@suse\\.de') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}')
...
suse_unseen:match_to('DL-(Prague|PRAGUE|TAG|SU(-ALLFY16)?|MicroFocusInternational|MICROFOCUSINTERNATIONAL)(-(A[lL]{2}|EMPLOYEES|Employees|Office))?(\\.iList\\.INTERNET)?@(ilist\\.attachmategroup\\.com|ILIST\\.ATTACHMATEGROUP\\.COM)'):move_messages(suse['newsletters'])
