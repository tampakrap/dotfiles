...
logs = suse_unseen:match_from('(HelpDesk@attachmategroup|((istdirteam|GWAVA|gwava|webmaster|pwnotify|SSC-EMEA)@|(psoft@(sybok|sarek)|WF-BATCH@eccprd1)\\.provo\\.)novell|helpdesk@netiq|support@ovationincentives|mailer@doodle|no-reply@accounts\\.google|(eServiceDesk|ssc-emea|guest|AssetManagement|rio.servicedesk|pwnotify)@microfocus|(orders|noreply)@eventbrite)\\.com|(swamp|orthos)_noreply@suse\\.de') + suse_unseen:match_from('maint-coord@(novell\\.com|suse\\.de)'):match_subject('Maintenance QA (SLA|KPI) watchdog report - [0-9]{8}')
...
newsletters = suse_unseen:match_to('DL-(Prague|PRAGUE|TAG|SU(-(ALLFY16|ENG))?|MicroFocusInternational|MICROFOCUSINTERNATIONAL|HeritageMF)(-(A[lL]{2}|EMP(LOYEES)?|Employees|Office|OFFICE))?(\\.iList\\.INTERNET)?@((ilist|ILIST)\\.)?(microfocus|MICROFOCUS)\\.(com|COM)') + suse_unseen:contain_from('einfo@suse.com')
